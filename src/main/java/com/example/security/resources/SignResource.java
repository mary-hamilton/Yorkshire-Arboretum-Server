...
public class SignResource {
    ...
    public ResponseEntity<SignDto> addNewSign(
        ...
        SignDto signDto = new SignDto(title, description, lat, lon);
        signService.addNewSign(signDto);
        return new ResponseEntity<>(signDto, HttpStatus.CREATED);
    }
    ...
    public ResponseEntity<SignDto> editSign(
            ...
            @RequestParam double lon) {
        SignDto signDto = new SignDto(title, description, lat, lon);
        signService.editSign(signId, signDto);
